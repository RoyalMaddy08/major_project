...
_last_conn_attempt = 0

def check_db_connected():
    global db_client, db, mongodb_connected, _last_conn_attempt
    if mongodb_connected and db is not None:
        return True
    
    now = time.time()
    if now - _last_conn_attempt < 10:
        return mongodb_connected
        
    _last_conn_attempt = now
    try:
        db_client = MongoClient(mongodb_uri, serverSelectionTimeoutMS=1500)

...
def _ensure_db():
    """Lazy MongoDB connection with time-gated retry (max once per 10s). Safe to call on every request."""
    global mongodb_connected, db_client, db, _last_db_attempt
    if mongodb_connected and db is not None:
        return True
    now = time.time()
    if now - _last_db_attempt < 10.0:
        return False
    _last_db_attempt = now
    try:
        db_client = MongoClient(Config.MONGODB_URI, serverSelectionTimeoutMS=3000)
...
if __name__ == '__main__':
    # Initialize config singleton in MongoDB if not present
    if _ensure_db():
        try:
            if db[Config.MONGODB_STATE_COLLECTION].find_one({'_id': 'singleton'}) is None:
                ...
                print("Battery State Estimator — Baseline configuration inserted to MongoDB.")
        except Exception as e:
            print(f"Battery State Estimator — DB init error: {e}")

    print(f"Battery State Estimator — Physics Engine running on http://localhost:{Config.PORT}")

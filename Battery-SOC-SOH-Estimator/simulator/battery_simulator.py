...
class BatterySimulator:
    ...
    def step(self, current, dt, accelerated_aging=False):
        """
        ...
        dSOC = (current * dt) / (active_capacity * 3600.0)
        ...
        dV1 = ((current - self.V1 / R1) / C1) * dt
        dV2 = ((current - self.V2 / R2) / C2) * dt
        self.V1 += dV1
        self.V2 += dV2

        # 4. Thermal Model Update (Joule Heating + Convective Cooling)
        # Heat generated: P_loss = I^2 * R0 + I * V1 + I * V2
        heat_generated = (current * current * R0) + abs(current * self.V1) + abs(current * self.V2)
        cooling = self.cooling_coefficient * (self.temperature - self.T_ambient)
        dT = ((heat_generated - cooling) / self.thermal_capacitance) * dt
        self.temperature = max(self.T_ambient, self.temperature + dT)
        ...
        current_aging_factor = np.power(abs(current), 1.3)
        ...
        terminal_voltage = ocv + (current * R0) + self.V1 + self.V2
    ...
    def add_sensor_noise(self, state, v_noise=0.005, i_noise=0.05, t_noise=0.2):
        """
        Add measurement noise to simulate real-world physical sensors.
        """
        return {
            'time': state['time'],

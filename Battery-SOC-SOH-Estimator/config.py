...
class Config:
    ...
    # Maximum real-time elapsed (in sim steps) before the catch-up loop is clamped
    # to avoid serverless timeout storms on cold starts or long inactivity.
    SIM_MAX_CATCHUP_STEPS = int(os.environ.get("SIM_MAX_CATCHUP_STEPS", 100))

    # If elapsed real time exceeds this many sim steps, truncate to SIM_TRUNCATE_STEPS
    SIM_CATCHUP_THRESHOLD = int(os.environ.get("SIM_CATCHUP_THRESHOLD", 200))
    SIM_TRUNCATE_STEPS = int(os.environ.get("SIM_TRUNCATE_STEPS", 100))
